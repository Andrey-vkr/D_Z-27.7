...
std::string Chat::registerUser()
{
    ...
    {
        std::cout << "A new user has been registered:" << login << std::endl;
...
std::string Chat::addMessage()
{
    ...
    std::thread readerThread([&logger]()
        {
            std::string lastLine = logger.readLog();
            std::cout << "Last Line from Log: " << lastLine << "\n";
        });
